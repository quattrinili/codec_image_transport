...
void H264Subscriber::internalCallback(const sensor_msgs::CompressedImage::ConstPtr &message,
  ...
  convert_ctx = sws_getCachedContext(convert_ctx, picture->width, picture->height,
                                     AV_PIX_FMT_YUV420P, picture->width, picture->height,
  ...
  sws_scale(convert_ctx, (const uint8_t *const *)picture->data, picture->linesize, 0,
            picture->height, &dst, &stride);
